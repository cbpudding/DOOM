    ...
    for tick in (1..ticks).rev() {
        for i in 0..width as _ {
            unsafe {
                if *Y.offset(i) < 0 {
                ...
                } else if *Y.offset(i) < height {
                    let mut dy = if *Y.offset(i) < 16 {
                        *Y.offset(i) + 1
                    } else {
                        8
                    };
                    if *Y.offset(i) + dy >= height {
                        dy = height - *Y.offset(i);
                    }
                    // ...
                    let mut idx = 0;
                    for j in (1..dy).rev() {
                    ...
                    *Y.offset(i) += dy;
                    // ...
                    idx = 0;
                    for j in (1..(height - *Y.offset(i))).rev() {
                        // ...
                        idx += width;
                    }
                    done = false;
...
pub extern "C" fn screen_wipe(wipeno: c_int, x: c_int, y: c_int, width: c_int, height: c_int, ticks: c_int) -> c_int {
    let wipes = [
        init_color_x_form,
        do_color_x_form,
        exit_color_x_form,
